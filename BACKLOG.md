...
- [x] **34. Marqueur 🛒 sur le calendrier** — ✅ FAIT (b131). Repère 🛒 sur chaque jour où un spectacle a des consommables en attente (`specHasItems` par jour dans `renderCalendar`).
- [x] **35. Historique des consommables** — ✅ FAIT (b131). « ✓ Racheté » archive dans un champ `history` (label, signalé par, racheté par + date) → section dépliable « 🧾 Historique des rachats » dans la fiche. **Règle Firestore `specSheets` (champ `history`) à publier.**
...
- [x] **37. Photos dans la fiche spectacle** — ✅ FAIT (b131). Section « 📷 Photos / plan de feu » : matche le nom du spectacle avec un sous-dossier du dossier Drive `1eGBxIRyvaRRkPEiVHvxCF21gwiGjAl6m`, charge les images en vignettes (clic = ouvre dans Drive). Pas de Firebase Storage — 100 % Drive. **b134** : bouton 📷 pour ajouter une photo depuis le téléphone (caméra/galerie) → réduite en JPEG ≤2000 px → uploadée dans le sous-dossier (créé si absent). ⚠️ Le compte Google connecté doit avoir accès au dossier ; les sous-dossiers doivent être nommés comme les spectacles.
...
- [x] **40. Session Google longue (Cloudflare)** — ✅ VALIDÉ (iPhone, 2026-07-06). Reste connecté ~7 j sans reconnexion.
